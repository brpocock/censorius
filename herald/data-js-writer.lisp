...
values. Descends plists recursively. Coerces keys to strings"
...
(defmethod handle-verb ((verb (eql :data)))
  (let* ((invoice (field :invoice)))
    (format t "Content-Type: application/javascript~%~%")
    (st-json:write-json (apply #'st-json:jso*
                               (append (list
                                        :festival (st-json:jso* :season (next-festival-season)
                                                                :year (next-festival-fear))
                                        :prices (read-prices))
                                       (read-invoice invoice))) t)))
