...
(with-sql
  (dolist
      (state
        ...
        (remove-if-not
         (lambda (thing)
           (and (equal (getf thing :country) "USA")
                (= 2 (length (getf thing :abbreviation)))))
         (mapcar
          (lambda (thing)
            (mapcar (lambda (column)
                      (when (getf thing column)
                        (setf (getf thing column) (parse-integer (getf thing column) :junk-allowed t))))
                    '(:id :fips-state :census-region :census-division :circuit-court))
            (setf (getf thing :standard-federal-region) (roman-number-value (getf thing :standard-federal-region)) )
            (setf (getf thing :relationship) (ecase (keyword* (getf thing :type))
                                               (:state "UNION")
                                               (:military "MIL")
                                               (:commonwealth "COMMONWEALTH")
                                               ((:minor :territory) "TERRITORY")
                                               (:capitol "CAPITAL")))
            (setf (getf thing :style) (ecase (keyword* (getf thing :type))
                                        (:state (if (member (getf thing :abbreviation) '("MA" "PA" "KY" "VA") :test #'string-equal)
                                                    "COMMONWEALTH"
                                                    "STATE"))
                                        (:military "APO")
                                        (:commonwealth "COMMONWEALTH")
                                        ((:territory :capitol :minor) "TERRITORY")))
            thing)
          (mapcar
           (lambda (thing)
             (interleave '(:id :name :abbreviation :country :type :sort :status :occupied :notes :fips-state :assoc-press
                           :standard-federal-region :census-region :census-region-name :census-division :census-division-name :circuit-court) 
                         thing))
    ...
    (apply #'db-query "insert into `us-states`
   (`postal-abbreviation`,`short-name`,`official-name-english`,`official-name`,`style`,`relationship`,`ap-abbreviation`,`fips-state`,
    `federal-region`,`census-region`,`census-division`,`circuit-court`)
   values (?,?,?,?,?,?,?,?,?,?,?,?)"
           
           (mapcar (curry #'getf state) '(:abbreviation :name :name :name :style :relationship :assoc-press :fips-state
                                          :standard-federal-region :consus-region :census-division :circuit-court)))))
